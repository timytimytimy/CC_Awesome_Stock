...
## 🔴 选股漏斗重构（高优先 — 关系系统能否"提前预测"）

> 2026-05-21 weekly_pick 实跑暴露：`screen_all_market.py` 的初筛闸门与系统
> "提前预测、抓未启动机会、可接受 3-6 个月周期"的根本目标**直接矛盾**。
> **建议优先级高于 P2-P7。**

**问题**：当前漏斗 5518 →（流动性/ST 过滤）1399 → **（`base_score`=成交额+当日涨幅）80** → 30 → 深度分析。
1399→80 这道闸门**只看当日量价**，财报/估值/趋势/宏观/政策全部要到 enrich 之后才算。
后果：基本面好、但当天走平/微跌、成交额一般的"未启动"标的，在 enrich 前就被砍掉，
永远进不了基本面评分——而"未启动的好票"恰恰是 3-6 个月策略要抓的。等于前面搭的
知识库/宏观/政策分析，在最关键的选股环节被一道当日动量闸门架空。

**优化项**：

- [ ] **重写 `base_score` 初筛去动量化**：去掉"当日涨幅"。改用快照免费字段——
  成交额（流动性闸门）、PE/PB（估值合理性）、60 日 / 年初至今涨跌幅（中期位置，
  用"不过热"钟形评分而非"越高越好"）。先确认 spot 缓存 CSV 是否保留这些列，没有就补存。
- [ ] **放大 / 取消有损初筛**：`--enrich` 默认 80（仅占 1399 的 6%）太小。对"周频、
  3-6 个月周期"的定位，应直接 enrich 全部 1399 只——彻底取消有损初筛。enrich 网络密集，
  但周频一次 30-50 分钟可接受；需加分批 + 失败重试 + 断点续跑 + 缓存复用保证可靠。
- [ ] **多镜头筛选（对接能力路由表）**：`screen_all_market` 当前只产出一个动量偏向的
  综合分。改为多套打分 profile——深度价值镜头（低 PE/PB 分位+高 ROE+健康 CFO）、
  产业景气镜头、困境反转镜头——分别捞不同候选，对应 capability-matrix 的 stock_type。
- [ ] **宏观/行业/政策喂进个股得分**（吸收原 P5"宏观到行业评分联动"）：阶段 1-2 的
  信用周期、行业宏观适配分、政策催化必须成为个股得分的加减项。让选股从"纯自下而上扫描"
  变成"自上而下预测 + 自下而上验证"——这才是"提前预测"的真正来源。
- [ ] **回归测试** `tests/data/test_screen_funnel.py`：核心场景=一只低估值、当日走平、
  基本面优的股票不被初筛漏掉。
