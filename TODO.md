...
### P5.1 数据源丰富与冗余 ⭐（2026-05-22 提出 — 本轮多次踩坑）

> 本轮反复栽在数据源上：CPI/M2/社融的金十源冻结数月、东财 EM 接口常被墙、
> 美联储利率无当期源、北上资金被交易所停止披露。数据闸门能**识别**问题，
> 但识别 ≠ 有数据。系统目前单一依赖 akshare，是明显的单点风险。

- [ ] **主源 + 备源机制**：为每个关键数据源定义主源和备源，主源失效自动回退
      （`load_market_snapshot` 已部分这样做，需推广到宏观/财报/行业各 getter）。
- [ ] **接入第二家数据商作冗余**：tushare（需 token，README 已列为可选增强）；
      必要时直连官方源（统计局 / 央行 / 交易所）。
- [ ] **优先补的具体缺口**：
  - 美联储利率当期源（`macro_bank_usa_interest_rate` 冻结在 2025-07）
  - 社融广义口径（现仅用新增人民币贷款代理；社融含债券/股权/影子银行更全）
  - 业绩预告 / 财报的非东财备源（东财常被墙）
  - 北上资金替代视角（实时净额已停披露，可考虑陆股通持股变动等间接指标）
- [ ] **provider 标注**：`data_health.py` 每个数据源标出当前用的是哪家，便于排查。
- [ ] **数据源巡检**：定期（如每月）自动跑 `data_health.py`，接口冻结早发现。
