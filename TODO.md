...
### P6.1 典型分析案例库（`kb/cases/`）⚠️ 当前空白

- [ ] 用蒸馏出的方法论 + 真实历史数据，构建端到端的完整分析案例。
- [ ] 优先案例（来自 `kb/cases/_index.md` 建议）：
  - 贵州茅台 600519 — 长期价值派完整周期
  - 白酒板块 2013 反腐打压 — 逆向赔率布局
  - 某半导体股 — 趋势成长派 CANSLIM 完整周期
  - 某医药股集采冲击 — 困境反转 vs 价值毁灭
- [ ] 每个案例完整还原"市场环境 → 行业 → 个股 → 买卖 → 结果"，作为分析模板。
- [ ] 决策权重 `journal/lessons > kb/cases > playbooks`，cases 是重要学习样本，不应空白。

### P6.2 机构作者档案扩充

- [ ] P1 待建 6 位：张坤、朱少醒、但斌、董宝珍、李蓓（半夏，宏观对冲）、Peter Lynch。
- [ ] P2 待建：姜诚（中泰，"慢就是快"）。
- [ ] 当前 11 位核心作者已够用，扩充用于丰富多流派交叉验证。
