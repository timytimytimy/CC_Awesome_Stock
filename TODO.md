...
## 🔴 系统闭环：从"扫描器"到"预测系统"（最高优先）

> 2026-05-22 复盘：系统目前是"扫描器"（回答"此刻哪些股票看起来好"），
> 还不是"预测系统"（"哪些东西正在变好、何时会被市场发现"）。差距全在**时间维度**——
> 没有记忆、没有反馈、领先信号不驱动任何东西。**在加更多扫描能力之前，先建闭环。**
...
- [ ] FOMO/追高/亏损加仓检测；连续亏损后冷静期；月度调仓次数上限
...
- [ ] **宏观到行业评分联动**（→ 已并入上方"🔴 选股漏斗重构"统筹，此处保留实现细节）：让 `snapshot_macro.py` 的信用周期、利率、通胀和流动性结论真正影响 `snapshot_industry.py` / `screen_all_market.py` 的行业与个股排序。
