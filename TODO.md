...
- [ ] **Layer 4 · cninfo 作财报/公告权威主源**（里海数据源调研，2026-05-22）：
      财报/公告主源从 akshare（东财/同花顺**转手加工版**）升级为**巨潮资讯 cninfo**
      （证监会指定法定披露平台、原始源）；akshare 有 cninfo 接口可直接用。
      属真升级（同类数据换权威源，非范围扩张），但是 build——**排在系统验证之后**。
      配套：交易所问询函作为风险信号源。
