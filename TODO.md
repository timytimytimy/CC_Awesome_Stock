...
- [ ] **追踪自动化**（收盘后定时复查 + 触发推送）。
- [ ] **组合层约束**（待 journal/trades 有真实持仓后启用，自原 P2 移入）：推荐一组候选时
      不超单行业上限、与现有持仓做主题相关性检查——避免组合过度集中。单股/单行业上限本身
      已在 `personal-profile.yaml` 的 `position_rules`，缺的是"组合级"统筹。
...
- [x] **Layer 4 · cninfo 官方披露源接入**（2026-05-22 完成）：
      `stock_data/cninfo.py` + `scripts/cninfo_query.py`——公司概况、公告披露、
      招股说明书定位、交易所监管函扫描，全部直查巨潮（证监会法定披露原始源）。
      已接入 decision-tree 阶段4（risk 扫描必做、年轻公司读招股书）。回归测试 10 项。
- [ ] （后续）把 `financial_check.py` 的财报主源也从 akshare 切到 cninfo——
      更彻底的权威性升级，可排在系统验证之后。
- ~~tushare~~：免费版积分受限且有停运先例，对"免费优先"定位性价比低，保持可选。
- ~~AlphaEngine~~：实测仅对机构开放、个人无法访问，且是研报/纪要聚合（券商观点类），排除。
- [x] **取数方法论先行**：里海"源优先级 / 招股书 / 问询函"取数纪律已写入
      `kb/playbooks/lihai-stock-research-checklist.md`（免费、零成本、立即生效）。
...
### P5.3 数据预热层 ✅ 已完成（2026-05-22）

> 痛点：weekly_pick 每次都重新取数，全市场增强约 30 分钟——这种摩擦会扼杀
> "每周跑 + 验证命中率"的习惯。预热把那 30 分钟从"你坐着等"挪到"盘后无人值守
> 时跑掉"，分析时秒级读缓存。

- [x] `data/scripts/refresh_data.py`：预热编排——按序跑 macro/policy/market/industry/
      earnings/data_health + `screen_all_market --enrich 0`，灌满现有缓存。
- [x] `--quick`（只快数据，实测 ~1 分钟）/ 默认全量（含全市场增强）/ `--only <key>` 三种用法；
      每任务计时、超时保护、统一日志 `data/.cache/refresh_<date>.log`。
- [x] 手动执行入口（用户每天盘后自己跑）；调度按要求暂不做。
- [x] 未重写 8 个分散缓存（能用）；深度分析逐股数据不预取（候选未知前无法预取）。
- [ ] （后续可选）launchd/cron 定时调度。
