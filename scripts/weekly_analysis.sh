...
7. **【强制】阶段 4 每只候选股必须先跑 data/scripts/financial_check.py 拿财报体检**
   - 必须在报告 C 段列出：总分（/100）+ 五维评分 + 红旗清单 + CFO/净利润 + 5 年财务摘要
   - 财报体检总分 < 50 → 强制"放弃"
   - 总分 50-69 → 最高 C 档（跟踪事件），禁止 A/B 档
   - CFO/净利润 < 0.5 → 必须显式标记为"利润可能虚增"重大风险
8. 对每只候选股调用 data/scripts/snapshot_stock.py
9. 对每只候选股调用 data/scripts/find_similar_cases.py
10. 严格按 A/B/C/D/E 五段格式输出完整报告
11. 每个判断必须标注 [事实/推断/假设/情绪/传闻]
12. 报告开头写：# 周报 $DATE
13. 最多选出 6 只候选股进入 C 段
14. **【强制】若宏观信用周期=衰退或市场温度=极冷，最高只能给 B 档候选，禁止 A 档**
